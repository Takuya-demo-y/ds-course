...
| # | やること | 難易度 | 時間 | 累計 |
|---|---------|--------|------|------|
| 準備 | ライブラリ読み込み・データ読み込み | — | 5分 | 5分 |
| データ確認 | `head` / `info` / `isnull` / `describe` の4点確認 + チェックポイント記入 | — | 7分 | 12分 |
| **問1** | 品種ごとのデータ件数を確認する（`value_counts`） | ★☆☆ | 7分 | 19分 |
| **問2** | 品種別の集計表を作る（`groupby`） | ★★☆ | 11分 | 30分 |
| **問3** | 集計結果から考察する（AI 禁止） | ★★☆ | 10分 | 40分 |
| 問4 | 条件でデータを絞り込む（発展） | ★★★ | 余り次第 | — |

> ✅ **問3まで完了すれば合格です。問4は時間が余った人向け**
...
【使うライブラリ】pandas
【データの形】178行・「品種名」列に wine_0/wine_1/wine_2 が入っている
